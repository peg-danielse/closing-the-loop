...
def read_history(label, base_label):
    history_df = pd.read_csv(PATH + f"/output/{base_label}/{label}/" + f'{label}_stats_history.csv')
...
def read_response(label, base_label):
    resp_df = pd.read_csv(PATH + f"/output/{base_label}/{label}/" + f'{label}_responce_log.csv')
...
def read_traces(label):
    data = {}
    with open(PATH + f"/output/{base_label}/{label}/" + f'{label}_traces.json', 'r') as file:
        data = json.load(file)
...
def read_metrics(label):
    metrics = {}
    for n in glob.glob(PATH + f"/output/{base_label}/{label}/" + f'{label}_*_metrics.csv'):
